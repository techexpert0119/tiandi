...
<html lang="en"> 
  <head>
  <?
  globvars('go','action','rates','del');
  $na = ($globvars['action'] == 'add') ? 'Save twice to add rates' : '';
  $globvars['debug'] = 0 ;
  $globvars['sq_table'] = 'ship_options'; // table name
  ...
  $globvars['sq_keys'] = array('lkua','lec','le'); // field keys
  $globvars['sq_names'] = array(' ','Order','Name'); // field names
  $globvars['sq_notes'] = array('',$na); // field notes
  ...
  $globvars['sq_heads'] = array('','','','Costs'); // break headings (where 'b')
  $globvars['sq_style'] = array(); // style override

  $globvars['st_ids'] = array('','',''); 

  if($globvars['action'] == 'delete') {
    my_query("delete from `ship_rates` where `so_id` = '{$globvars['del']}'");
  }

  if($globvars['go'] && is_array($globvars['rates']) && count($globvars['rates'])) {
    my_query("delete from `ship_rates` where `so_id` = '{$globvars['go']}'");
    foreach($globvars['rates'] as $st_id => $sr_price) {
      $string = "insert into `ship_rates` set `so_id` = '{$globvars['go']}', `st_id` = '{$st_id}', `sr_price` = '{$sr_price}'";
      // print_p($string);
      my_query($string);
    }
  }
  
  $string = "select * from `ship_rates`";
  $query = my_query($string);
  while($row = my_assoc($query)) {
    $globvars['ship_rates'][$row['so_id']][$row['st_id']] = $row['sr_price'];
  }
  
  if($globvars['action'] == 'edit' || $globvars['action'] == 'delete' || ! $globvars['action']) {
    $string = "select * from `ship_types` left join `ship_regions` on `ship_types`.`sr_id` = `ship_regions`.`sr_id` order by `ship_regions`.`sr_order`, `ship_types`.`st_order`";
    $query = my_query($string);
    $k = 0 ;
    while($row = my_assoc($query)) {
      $globvars['sq_keys'][] = $k ? 'lez' : 'blez';
      $globvars['sq_names'][] = $globvars['action'] == 'edit' ? $row['st_name'] : $row['st_code'];
      $globvars['sq_funct'][] = 'ship_type';
      $globvars['st_ids'][] = $row['st_id'];
      $k++;
    }
  }
  ...
  $globvars['sq_dsort'] = 'so_order'; // default sort (reverse _DESC)
  $globvars['sq_ajoin'] = "" ; // join filter string for list
 
  $globvars['plogo'] = $globvars['admin_logo'] ; // logo
  $globvars['ptitle'] =  'Shipping Options' ; // page title
  ...
  function ship_type() {
    ...
    $so_id = $globvars['i_row']['so_id'];
    $st_id = $globvars['st_ids'][$globvars['c_row']['col']];
    $price = number_format( (isset($globvars['ship_rates'][$so_id][$st_id]) ? $globvars['ship_rates'][$so_id][$st_id] : 0) ,2);
    if($globvars['action'] == 'edit' || $globvars['action'] == 'add') {
      ?>
      <input name="rates[<?= $st_id ?>]" type="text" value="<?= $price ?>" style="width:50px">
      <?
    }
    else {
      print $price;
