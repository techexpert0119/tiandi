...
<html lang="en"> 
  ...
  <?
  $globvars['debug'] = 0 ;
  $globvars['sq_table'] = 'ship_types'; // table name
  ...
  $globvars['sq_keys'] = array('lkua','leo','lec','le','le','le'); // field keys
  $globvars['sq_names'] = array(' ','Region','Order','Code','Name','Description'); // field names
  ...
  $globvars['sq_dsort'] = 'sr_id'; // default sort (reverse _DESC)
  $globvars['sq_ajoin'] = "left join `ship_regions` on `ship_types`.`sr_id`=`ship_regions`.`sr_id`" ; // join filter string for list
 
  $globvars['plogo'] = $globvars['admin_logo'] ; // logo
  $globvars['ptitle'] =  'Shipping Types' ; // page title
