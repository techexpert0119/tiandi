...
<html lang="en"> 
  ...
  if($globvars['action'] == 'edit' && $globvars['go']) {
    $notebutt1 = '<a style="width:110px" href="shop_subs.php?filter=c_id|[[c_id]]" target="shop_subs.php">EDIT SUBS</a>';
  }

  $globvars['debug'] = 0 ;
  $globvars['sq_table'] = 'shop_cats'; // table name
  ...
  $globvars['sq_names'] = array(
    ' ','URL','Redirect','Order','Visible','Menu',
  ...
  $globvars['sq_export'] = array(
    'Ref','URL','Redirect','Order','Visible','Menu',
    'Meta Title','Meta Description','Meta Keywords',
  ...
  $globvars['sq_dsort'] = 'c_order'; // default sort (reverse _DESC)
  $globvars['sq_ajoin'] = "" ; // join filter string for list
 
  $globvars['plogo'] = $globvars['admin_logo'] ; // logo
  $globvars['ptitle'] =  'Shop Categories' ; // page title
  ...
  $globvars['publicfld'] = 'c_url' ; // public page field or array
  ...
  function url() {
    ...
    if($go || ($action == 'add')) { // edit form
      ...
      print $globvars['main_root']['pages_shop'] . '/';
      ?>
      <input type="text" name="c_url" id="id_c_url" size="30" maxlength="30" value="<?= $dval ?>" onchange="fldchg++;" autocomplete="off">
      <?
    }
    elseif($action == 'export') {
      print $globvars['main_root']['pages_shop'] . '/' . $dval ;
    }
    else { // item list
      ?>
      <a target="public" href="<?= $globvars['base_href'] . $globvars['main_root']['pages_shop'] . '/' . $dval ?>"><?= $globvars['main_root']['pages_shop'] . '/' . $dval ?></a>
      <?
    }
  }
  ...
  function preview() {
    ...
    if($i_row['c_url']) {
      ?>
      <div style="float:right"><a style="width:110px;" target="public" href="<?= $globvars['base_href'] . $globvars['main_root']['pages_shop'] . '/' . $i_row['c_url'] . "?preview={$globvars['sessmd']}" ?>">PREVIEW</a></div>
      <?
    }
  }
  ...
  function listbutts() {
    ...
    ?>
    <span class="button">
      <a href="<?= $globvars['base_href'] . $globvars['main_root']['pages_shop'] . '/' . $i_row['c_url'] . "?preview={$globvars['sessmd']}" ?>" target="public">PREVIEW</a> &nbsp;
      <a target="shop_subs" href="shop_subs.php?filter=c_id|<?= $i_row['c_id'] ?>">EDIT SUBS</a> 
    </span>
    <?
  }
