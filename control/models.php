...
<html lang="en"> 
  <head>
  <?
  $string = "select * from `pages_subp` where `p_id` = 3";
  $query = my_query($string);
  while($row = my_assoc($query)) {
    $globvars['models'][$row['q_id']] = $row;
  }
  $_POST['sort'] = $_GET['sort'] = '';

  $globvars['public'] = '../' ; // public page
  globvars('action','go');
  if(($globvars['action'] == 'edit') && isset($globvars['models'][$globvars['go']])) {
    $globvars['public'] = '../systems/' . $globvars['models'][$globvars['go']]['q_url'] ; 
  }

  $globvars['debug'] = 0 ;
  $globvars['sq_table'] = 'models'; // table name
  ...
  $globvars['sq_names'] = array(
    '','Model','Other Name',
    'Light','Dark',
    'Slide/Button','Page Header',
    'Slide Main','Slide Mobile','Slide Title 1','Slide Title 2',
    'Button Tagline','Button Text',
    'Heading','Text','Image',
    'Heading','Text','Image',
    'Heading','Text','Image',
    'Heading','Text','Image',
  ...
  $globvars['sq_notes'] = array(
    '','<a style="width:110px" href="pages_subp.php?action=edit&amp;go=[[m_id]]" target="pages_subp.php">EDIT PAGE</a>','',
    '','',
    '','',
    '','','Part of [text] in square brackets<br>changed to light colour above','',
    'Home/About','About Us',
    'Section hidden if blank','','400x340',
    'Section hidden if blank','','400x340',
    'Section hidden if blank','','400x340',
    'Section hidden if blank','','400x340',
    'Section hidden if blank','','400x340'
  ...
  $globvars['sq_fpath'] = array(
    '','','',
    '','',
    'symb/slide','symb/page',
    'slide/main','slide/mob','','',
    '','',
    '','','comp',
    '','','comp',
    '','','comp',
  ...
  $globvars['sq_ajoin'] = "left join `pages_subp` on `pages_subp`.`q_id` = `models`.`m_id`" ; // join filter string for list
 
  $globvars['plogo'] = isset($globvars['admin_logo']) ? $globvars['admin_logo'] : 'logo.png' ; // logo
  $globvars['ptitle'] =  'Model Details' ; // page title
  ...
  function related() {
    ...
    if(isset($globvars['models'][$i_row['m_id']])) {
      print $globvars['models'][$i_row['m_id']]['q_menu'] ? $globvars['models'][$i_row['m_id']]['q_menu'] : $globvars['models'][$i_row['m_id']]['q_url'];
    }
  }
  ...
  function simage() {
    ...
    if((! $action) && $imgv && file_exists($imgv1 = $pthv . $imgv)) {
      ...
      if($imgp && file_exists($imgp1 = $pthp . $imgp)) {
        ...
        <? } else { $poph = 0 ; } ?>
        <img src="<?= clean_url($imgv1); ?>" style="<?= 'max-height:' . $imgh . 'px; max-width:' . ( $imgh * 2 ) . 'px' ; ?>; display:block;" alt="" border="">
        <? if($poph) { ?>
