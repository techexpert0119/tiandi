...
<html lang="en"> 
  ...
  globvars('done');
  if($globvars['done']) {
    $_POST['to'] =  url_noslash($_POST['to']);
    $_POST['from'] =  url_noslash($_POST['from']);
  }

  $globvars['debug'] = 0 ;
  $globvars['sq_table'] = 'redirects'; // table name
  ...
  $globvars['sq_keys'] = array('lkua','lec','le','le','lez'); // field keys
  $globvars['sq_names'] = array('','','','','TEST'); // field names
  ...
  $globvars['sq_funct'] = array('','','','selectto','testredirect'); // call functions
  ...
  $globvars['ptitle'] =  'Redirects (301)' ; // page title
  ...
  function selectto() {
    ...
    if($go || ($action == 'add')) { // edit form
     ...
     ?>
     <select name="to" class="chosen-select" size="1" style="width:500px;">
      <option value="">home page</option>
      <? foreach($globvars['pages_urls'] as $url) { ?>
      <option value="<?= optsel($url,$i_row['to']) ?>"><?= $url ?></option>
      <? } ?>
     </select>
     <?
    }
  ...
  function testredirect() {
    global $globvars; extract($globvars);
    if(in_array($i_row['to'], $globvars['pages_urls'])) {
      ?>
      <div class="button"><a href="<?= '../' . $i_row['from'] ?>" target="checkredirect">GO</a></div>
      <?
    }
    else {
      print 'INVALID TO';
