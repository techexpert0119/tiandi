...
<html lang="en"> 
  ...
  if($globvars['action'] == 'edit' && $globvars['go']) {
    $notebutt1 = '<a style="width:110px" href="pages_main.php?action=edit&amp;go=[[p_id]]" target="pages_main.php">EDIT MAIN</a>';
    $notebutt2 = '<a style="width:110px" href="pages_subs.php?filter=q_id|[[q_id]]" target="pages_subs.php">EDIT SUBS</a>';
    $string = "select * from `pages_subp` where `q_id` = '{$globvars['go']}' limit 1";
  ...
  $globvars['debug'] = 0 ;
  $globvars['sq_table'] = 'pages_subp'; // table name
  ...
  $globvars['sq_notes'] = array(
    '',$notebutt1,'',ALT_URL_NOTE,'',$notebutt2,'preview()','prodlink()',
  ...
  $globvars['sq_lookf'] = array(
    '',"ORDER BY `p_order`",'','','','','',"WHERE `t_select` ORDER BY `t_select`",
    '','','','',
    '','','',
    '','','','','','',
    '','','','',
  ...
  $globvars['sq_dsort'] = 'p_url,q_order'; // default sort (reverse _DESC)
  $globvars['sq_ajoin'] = "left join `pages_main` on `pages_main`.`p_id` = `pages_subp`.`p_id`" ; // join filter string for list
 
  $globvars['plogo'] = $globvars['admin_logo'] ; // logo
  $globvars['ptitle'] =  'Sub Pages 1' ; // page title
  ...
  $globvars['publicfld'] = array('p_url','q_url') ; // public page field or array
  $globvars['publicfjn'] = "left join `pages_main` on `pages_main`.`p_id` = `pages_subp`.`p_id`" ; // join for publicfld
  ...
  head();
  @include_once('../scripts/wcolpick.inc.php');
  ?>
  ...
  function prodlink() {
    ...
    $res = 'REQUIRED';
    if($i_row['p_id'] == 3 && $i_row['q_template'] == 3) {
      $string = "select * from `models` where `m_id` = '{$i_row['q_id']}' limit 1";
      // print_p($string);
      $query = my_query($string);
      if(! my_rows($query)) {
        $string = "insert into `models` set `m_id` = '{$i_row['q_id']}'";
        // print_p($string);
        $query = my_query($string);
      }
      $res = '<div class="buttonr"><a style="width:110px;background-color:#D71A23" href="models.php?action=edit&amp;go=' . $i_row['q_id'] . '" target="models.php">MODEL DETAILS</a></div>';
    }
    print $res;
  }
  ...
  function url() {
    global $globvars; extract($globvars);
    // print_arv($globvars['lookups']['p_id']);
    if($go || ($action == 'add')) {
      if(isset($globvars['save'])) { $globvars['save']++; }
      ?>
      <select class="chosen-select" name="p_id" id="p_id" size="1" onchange="fldchg++;" style="width:150px;"> 
        <option value="">** Select **</option>
        <? foreach($globvars['lookups']['p_id']['sq_arr'] as $k => $a) { ?>
        <option value="<?= optsel($k,$i_row['p_id']) ?>"><?= $a['p_url'] ?></option>
        <? } ?>
      </select> 
      <span style="height:15px;display:inline-block;vertical-align:middle">/</span>
      <input type="text" name="q_url" id="id_q_url" size="30" maxlength="30" value="<?= $i_row['q_url'] ?>" onchange="fldchg++;" autocomplete="off" style="display:inline-block;vertical-align:middle">
      <?
    }
    elseif($action == 'export') {
      ...
      $r = isset($globvars['lookups']['p_id']['sq_arr'][$i_row['p_id']]) ? $globvars['lookups']['p_id']['sq_arr'][$i_row['p_id']]['p_url'] . '/' : '';
      ?>
      <a target="public" href="<?= $base_href . $r . $i_row['q_url'] ?>"><?= $r . $i_row['q_url'] ?></a>
      <?
    }
  }
  ...
  function preview() {
    global $globvars; extract($globvars);
    if(isset($globvars['pages_admin'][$i_row['p_id']]) && $globvars['pages_admin'][$i_row['p_id']]['p_url']) {
      ?>
      <div style="float:right"><a style="width:110px;" target="public" href="<?= $globvars['base_href'] . $globvars['pages_admin'][$i_row['p_id']]['p_url'] . '/' . $i_row['q_url'] . "?preview={$globvars['sessmd']}" ?>">PREVIEW</a></div>
      <?
    }
  }
  ...
  function pageopts() {
    ...
    elseif(isset($globvars['pages_admin'][$i_row['p_id']]) && $globvars['pages_admin'][$i_row['p_id']]['p_url']) {
      ?>
      <span class="button">
        <a href="<?= $globvars['base_href'] . $globvars['pages_admin'][$i_row['p_id']]['p_url'] . '/' . $i_row['q_url'] . "?preview={$globvars['sessmd']}" ?>" target="public">PREVIEW</a> &nbsp;
        <a target="pages_subs.php" href="pages_subs.php?filter=q_id|<?= $i_row['q_id'] ?>">EDIT SUBS</a> 
      </span>
      <?
    }
  }
  ...
  function form_foot() {
    global $globvars; extract($globvars);
    if($go && $i_row['q_template'] && isset($globvars['templates'][$i_row['q_template']]) && $globvars['templates'][$i_row['q_template']]['t_cms']) {
      $string = "select 
        `pages_main`.`p_id`, 
        `pages_subp`.`q_id` 
        from `pages_subp` 
        left join `pages_main` on `pages_main`.`p_id` = `pages_subp`.`p_id`
        where `pages_subp`.`q_id` = '{$i_row['q_id']}' limit 1
      ";
      // print_p($string);
      $query = my_query($string);
      if(my_rows($query)) {
        $globvars['page'] = my_assoc($query);
        $ta = explode("\r\n", $globvars['templates'][$i_row['q_template']]['t_cms']);
        foreach($ta as $cms) {
