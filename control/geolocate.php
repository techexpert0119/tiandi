...
<html lang="en"> 
  <head>
  <?
  globvars('check_ip','filter');
  $globvars['mfilter'] = '' ; // add master filter for query
  if($globvars['check_ip']) {
    geolocate($globvars['check_ip'],0);
    $globvars['mfilter'] = "`ip_address` = '{$globvars['check_ip']}'";
  }

  $globvars['debug'] = 0 ;
  $globvars['sq_table'] = 'geolocate'; // table name
  ...
  $globvars['sq_keys'] = array('lkua','lv','lv','lv','lv','lv','v','x','v'); // field keys
  $globvars['sq_names'] = array(' ','IP','Date','Country','Currency','Continent','Symbol','','Data'); // field names
  ...
  $globvars['sq_funct'] = array('','','','','','','','','geo_arr'); // call functions
  ...
  $globvars['sq_list'] = array(1,2,3,5,6,4,7,8,9); // column order array or '' for default
  ...
  $globvars['ptitle'] =  'Geolocate'; // page title
  ...
  <body> 
  <div style="position:absolute; top:50px; left:600px; z-index:99">
    <form method="post" action="geolocate.php"><b>CHECK IP</b> 
      <input type="text" id="check_ip" name="check_ip" value="<?= $globvars['check_ip'] ?>" class="corners" style=";width:100px;"> 
      <input type="submit" name="submit" class="submit" value="go" style=""> 
      <span id="searchx" class="black" style="display:inline-block; padding:0 0 0 5px;">
        <a onclick="$('#search').val('');$('#check_ip').val('');$('#lform').submit();return false;" href="#">&#10006;</a>
      </span>
    </form>
  </div>
  <?
  ...
  function geo_arr() {
    ...
    $out = '';
    if($dval) {
      if(substr($dval,0,2) == 'a:') {
        if($out = unserialize($dval)) {
          $out = objectToArray($out) ;
        }
      }
      else {
        $out = objectToArray(json_decode(base64_decode($dval))) ;
      }
    }
    if(is_array($out)) {
      print_arr($out) ;
  ...
  function _simage() {
    ...
    if((! $action) && $imgv && file_exists($imgv1 = $pthv . $imgv)) {
      ...
      if($imgp && file_exists($imgp1 = $pthp . $imgp)) {
        ...
        ?>
        <a onmousemove="<?= $omo ?>" onmouseover="<?= $omo ?>" onmouseout="<?= $omx ?>" onclick="<?= $omo ?>" href="#" style="display:inline-block; background-color:#000000; padding:5px;">
        <? } else { $poph = 0 ; } ?>
        <img src="<?= clean_url($imgv1); ?>" style="<?= 'max-height:' . $imgh . 'px; max-width:' . ( $imgh * 2 ) . 'px' ; ?>; display:block;" alt="" border=""> &nbsp; <?= $imgv ; ?>
        <? if($poph) { ?>
        </a><div id="<?= 'id_grid_' . $fname . $s ; ?>" style="display:none; position:absolute; border:solid 1px black; background-color:#000000; padding:5px; z-index:999">
        <img alt="" border="0" src="<?= clean_url($imgp1); ?>" height="<?= $poph ; ?>" style="display:block;">
        </div>
        <? } 
    }
    else {
